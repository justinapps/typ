<?php 
    ...
    if(substr($_SESSION['user']['email'],-7)=='@dcu.ie'){
       ...
       mysql_query("UPDATE `justinas_qdb`.`users` SET `online` = '25' WHERE `users`.`id` =".$_SESSION['user']['id']);
...
if (isset($_POST['NumRows']))
{
		...
		$sql="INSERT INTO `justinas_qdb`.`Lab25` (`lab25`,`Time`,`email`) VALUES  ('$ip',NOW(),'$email')";
	...
	if(isset($_POST['leave']))
	{
		...
		mysql_query("DELETE FROM `justinas_qdb`.`Lab25` WHERE `Lab25`.`email` ='$email'");
		mysql_close($conn);
	}
?>
